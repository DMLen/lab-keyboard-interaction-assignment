...
class Application(tk.Frame):
    def __init__(self, window_width, window_height, master=None):
        ...
        # adding a new event for triple click which tkinter apparently supports 
        self.canvas_keyboard.bind("<Triple-Button-1>", self.mouse_left_button_triple_click)
    ...
    def mouse_left_button_double_click(self, event):
        key = self.keyboard.get_key_pressed()
        current = self.label_word_candidates[0]
        self.label_word_candidates[0].config(text=current.cget("text") + key)

    # triple click for gesture solution 2 using tripleclick function
    def mouse_left_button_triple_click(self, event):
        key = self.keyboard.get_key_pressed()
        if key == "C":
            #copy function
            messagebox.showinfo("Message","Copy command inputted!")
            for i in range(len(self.label_word_candidates)): # clear the content of all word labels
                self.label_word_candidates[i].config(text='')
        elif key == "U":
            #undo function here
            messagebox.showinfo("Message","Undo command inputted!")
            for i in range(len(self.label_word_candidates)):
                self.label_word_candidates[i].config(text='')
        elif key == "S":
            #save function here
            messagebox.showinfo("Message","Save command inputted!")
            for i in range(len(self.label_word_candidates)):
                self.label_word_candidates[i].config(text='')
        elif key == "R":
            #redo function here
            messagebox.showinfo("Message","Redo command inputted!")
            for i in range(len(self.label_word_candidates)):
                self.label_word_candidates[i].config(text='')
        else:
            #this will happen if you triple-click any non-specified key
            messagebox.showinfo("Error","Invalid command inputted!")
            for i in range(len(self.label_word_candidates)):
                self.label_word_candidates[i].config(text='')

    # we also clearing candidates list because triple-clicking will add the key to the candidate list
